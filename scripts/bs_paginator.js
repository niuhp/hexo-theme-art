...
function bsPaginatorHelper(options){
  ...
  var currentPage = '<li class="page-item active"><span class="page-link">' +
  ...
  if (prevNext){
    if (current > 1) {
      ...
      result += '<li class="page-item disabled"><span class="page-link page-prev">' + prevText + '</a></li>';
    }
  }
  ...
  if (options.show_all){
    ...
    var spaceHtml = '<li class="page-item disabled"><span class="page-link page-space">' + space + '</span></li>';
  ...
  if (prevNext){
    if (current < total) {
      result += '<li class="page-item"><a class="page-link page-next" rel="next" ' +
              'href="' + link(current + 1) + '">' + nextText + '</a></li>';
    } else {
      result += '<li class="page-item disabled"><span class="page-link page-next">' + nextText + '</a></li>';
    }
  }

  return '<nav aria-label="archive navigation pagination"><ul class="pagination justify-content-center">' + result + '</ul></nav>';
}
